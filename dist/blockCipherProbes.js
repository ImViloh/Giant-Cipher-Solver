...
function deriveKeyVariants(password, byteLen) {
    const out = [];
...
function tryDecrypt(alg, data, key) {
    if (data.length < 8)
        return null;
    try {
        const iv = ivFor(alg);
        ...
        if (plain.length === 0 || plain.length > data.length * 20)
            return null;
        return plain;
    }
    catch {
 ...
 */
export function probeBlockCiphers(ciphertext, passwords, maxHits = 120) {
    const hits = [];
    const tried = new Set();
    const algorithms = [];
    const add = (name, kb) => {
        if (available.has(name))
            algorithms.push({ name, keyBytes: kb });
    };
    add("aes-128-ecb", 16);
    add("aes-192-ecb", 24);
    add("aes-256-ecb", 32);
    ...
    add("rc2-cbc", 8);
    add("rc2-cfb", 8);
    if (available.has("bf-ecb"))
        algorithms.push({ name: "bf-ecb", keyBytes: 16 });
    if (available.has("bf-cbc"))
        algorithms.push({ name: "bf-cbc", keyBytes: 16 });
...
export function probeBlockCiphersReversedInput(decoded, passwords, maxHits = 120) {
