...
function barMeterPct(pct, width) {
    const p = Math.max(0, Math.min(100, pct));
    const filled = Math.round((p / 100) * width);
    return (pc.green("█".repeat(filled)) +
        pc.dim("░".repeat(Math.max(0, width - filled))) +
        pc.dim(" ") +
        pc.bold(pc.white(p.toFixed(1))) +
        pc.dim("%"));
}
function buildBruteLiveLines(now) {
    ...
    if (snap.phase === "xor") {
        ...
        const mode = snap.vigMode === "beaufort-b64"
            ? "Beaufort"
    ...
    const phaseStart = snap.phase === "vig" && o.vigPhaseStartedAt != null
        ? o.vigPhaseStartedAt
        : o.xorPhaseStartedAt;
    const phaseElapsedSec = Math.max(0, (now - phaseStart) / 1000);
    const phaseTried = snap.phase === "vig" ? vigT : xorT;
    const phaseTotal = snap.phase === "vig" ? o.vigTotal : o.xorTotal;
...
function redrawBruteLive(lines) {
...
export function markVigBrutePhaseStart() {
    if (bruteLiveOpts) {
        bruteLiveOpts.vigPhaseStartedAt = Date.now();
    }
}
export function stopBruteLiveUi() {
    if (bruteLiveTimer) {
...
export function printPreSolveBanner(opts) {
    ...
    const row = (s) => console.log(pc.yellow("║") +
        " " +
        s +
    ...
    row(pc.dim("  Parallel workers (brute ranges) ") +
    ...
    row(pc.dim("  Layered probes: ") +
        pc.cyan("classical A26/A52, AES/3DES/RC2*/BF*, nested B64, zlib, nibble, RC4, XOR") +
        pc.dim(" (extended + layer-*)"));
    row(pc.dim("  Time budget (brute phases): ") +
        (opts.maxMs <= 0
            ? pc.cyan("unlimited") + pc.dim(" (GIANT_MAX_MS=0)")
            : pc.cyan(`${(opts.maxMs / 1000).toFixed(0)}s`)));
    if (opts.progressEvery > 0) {
        row(pc.dim("  Progress update every ") +
...
export function printXorBruteProgress(phase, tried, keyLen) {
    const pct = keyLen > 0
...
export function printVigBruteProgress(phase, tried) {
    console.log(pc.dim("  ⟳ ") +
...
export function printInterruptedProgress(snap, signal) {
    ...
    const row = (s) => console.log(pc.red("║") +
        " " +
        s +
        " ".repeat(Math.max(0, W - 4 - stripAnsi(s).length)) +
        " " +
    ...
    console.log("");
    console.log(top);
    row(pc.bold(pc.red("  INTERRUPTED (" + signal + ") — last brute-force progress")));
    console.log(mid);
    row(pc.dim("  Phase: ") +
        pc.white(snap.phase) +
        (snap.phase === "xor"
        ...
        (snap.phase === "vig"
            ...
            : ""));
    row(pc.dim("  XOR keys tried (cumulative): ") +
        pc.yellow(snap.xorTried.toLocaleString()));
    row(pc.dim("  Vigenère/Beaufort keys tried (cumulative): ") +
        pc.yellow(snap.vigTried.toLocaleString()));
    row(pc.dim("  Tip: re-run with the same env; progress is not saved to disk."));
    console.log(bot);
    console.log("");
}
