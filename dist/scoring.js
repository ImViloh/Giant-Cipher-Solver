...
const COMMON_BIGRAM_CODES = new Set();
...
function bigramScore(upper) {
    ...
    for (let i = 0; i < lim; i++) {
        ...
        if (c1 >= 65 && c1 <= 90 && c2 >= 65 && c2 <= 90) {
            if (COMMON_BIGRAM_CODES.has((c1 << 8) | c2))
                hits++;
        }
    }
...
export function bufferToPrintableUtf8(buf) {
    try {
...
export function bufferToLatin1(buf) {
