...
function indexOfCoincidenceBuffer(buf) {
...
function methodHistogram(candidates, topN) {
    const m = new Map();
...
export function buildCipherIntelligence(opts) {
    ...
    const iocSig = ioc === null
        ? 0.35
        : 1 - Math.min(1, Math.abs(ioc - ENGLISH_IOC) / 0.045);
    const lowIocSig = ioc === null ? 0.4 : ioc < 0.055 ? 1 - ioc / 0.055 : 0;
    const highEntropySig = decoded && decoded.length >= 32
        ? clamp01((entropy - 6.2) / (8 - 6.2))
        : 0;
    const blockAlignedSig = decoded && decoded.length >= 16 && decoded.length % 16 === 0 && entropy > 6.4
    ...
    predictions.push({
        ...
        confidence: toConfidence(0.55 + 0.35 * nestedSig + (zlibLikely ? 0.15 : 0)),
    ...
    predictions.push({
        ...
        confidence: toConfidence(0.3 * lowIocSig +
            0.35 * periodicitySig +
    ...
    predictions.push({
        ...
        confidence: toConfidence(0.25 * highEntropySig +
            0.35 * methodBoost("layer-rc4") +
            0.2 * methodBoost("rc4") +
            0.2 * (printable < 0.45 ? 0.7 : 0.2)),
        summary: "High-entropy binary with flat byte use often matches stream ciphers; solver probes RC4 with keyword-derived keys.",
        evidence: [
    ...
    predictions.push({
        ...
        confidence: toConfidence(0.45 * highEntropySig +
            0.3 * blockAlignedSig +
            0.35 * methodBoost("block-") +
            (hp?.finds.some((f) => f.title.includes("Repeated 16-byte")) ? 0.2 : 0)),
        summary: "Random-looking bytes, optional 16-byte alignment, and duplicate block hints point to block modes or unrelated binary.",
        evidence: [
            decoded
    ...
    predictions.push({
        ...
        confidence: toConfidence(0.35 * methodBoost("b64-rotate") +
            0.3 * methodBoost("atbash") +
            0.25 * methodBoost("reverse") +
            0.15),
        summary: "Community puzzles often permute the Base64 alphabet or reverse strings before standard decode — solver enumerates these.",
        evidence: [
            topMethods.some((t) => t.method.includes("b64-rotate") ||
    ...
    predictions.push({
        ...
        confidence: toConfidence(0.25 * iocSig * (1 - lowIocSig) * 0.8 +
            0.15 * methodBoost("classical-rail") +
            0.1 * methodBoost("reverse")),
        summary: "IOC can stay English-like under transposition while digraph statistics differ — rely on solver rail/reverse and dictionary hits.",
        evidence: [
    ...
    // --- Layer hypotheses ---
    const layerHypotheses = [];
    const chainA = ["Base64 (outer)"];
    if (nestedCount) {
    ...
    if (zlibLikely)
        chainA.push("zlib/gzip-compatible compressed blob");
    if (highEntropySig > 0.55)
        chainA.push("High-entropy inner (encryption or compressed noise)");
    layerHypotheses.push({
        rank: 1,
        chain: chainA,
        confidence: toConfidence(0.55 + 0.2 * nestedSig + (zlibLikely ? 0.15 : 0)),
        notes: "Treat as unwrap order: decode transports first, then attack cryptographic layers on the innermost high-signal buffer.",
    });
    const chainB = [
        "Base64 (outer)",
    ...
    layerHypotheses.push({
        ...
        notes: "Fits community workflows: transform outer string, then decode Base64 to bytes. Falsified if strict English only appears on unrelated method.",
    });
    const chainC = [
        "Base64 (outer)",
    ...
    layerHypotheses.push({
        ...
        notes: "Use when inner entropy is high and classical transforms stall — keyword-derived AES attempts matter.",
    });
    ...
    insights.push({
        ...
        interpretation: "High score indicates English-like statistics, not proof. Low spread between top candidates suggests ambiguity or layered noise.",
        falsify: "If ground-truth plaintext is known, compare method label — mismatch means missing transform or wrong alphabet.",
    });
    insights.push({
        title: "Index of coincidence on raw decoded bytes",
        observation: ioc !== null
        ...
        interpretation: ioc !== null && Math.abs(ioc - ENGLISH_IOC) < 0.02
        ...
        falsify: "Successful decode from a method that yields different letter statistics refutes reliance on raw IOC.",
    });
    insights.push({
        ...
        interpretation: entropy > 7.3 && printable < 0.4
        ...
        falsify: "A simple XOR/Vigenère path that jumps to strict English contradicts 'pure high-entropy ciphertext' without ruling out layering.",
    });
    insights.push({
        title: "Solver coverage",
        observation: `Top methods: ${topMethods.slice(0, 5).map((m) => `${m.method}×${m.count}`).join(", ") || "none"}.`,
        interpretation: "If the true method uses non-alphanumeric keys, custom base64 alphabets, or extra nesting depth beyond env limits, rankings may miss it.",
        falsify: "Manual trial of the actual method should dominate the candidate list once parameters match.",
    });
    insights.push({
        ...
        interpretation: "Near-misses can look readable in places; rely on the readability card and word scan for false positives.",
        falsify: "Human inspection of the alleged solution should read as coherent game narrative, not accidental n-grams.",
    });
    if (periodHint) {
        insights.push({
            title: "Structural period",
            observation: `Pattern scan flagged period ${periodHint} (byte-level repetition heuristic).`,
            interpretation: "Supports repeating-key XOR/Vigenère length guesses; could also be format framing.",
            falsify: "If brute keys at that length fail and English does not improve, period may be spurious.",
        });
    }
    return { predictions, layerHypotheses, insights, stats };
}
/** Plain text for session log. */
export function formatCipherIntelligenceForLog(report) {
    const lines = [];
    ...
    lines.push(`# ${"=".repeat(72)}`);
    lines.push(`IOC: ${s.indexOfCoincidence !== null ? s.indexOfCoincidence.toFixed(4) : "n/a"} (${s.letterCountForIoc} letters)  |  expected English ~${s.englishExpectedIoc}`);
    lines.push(`Entropy: ${s.byteEntropyBits.toFixed(2)} b/b  |  printable ASCII: ${(s.printableAsciiRatio * 100).toFixed(1)}%  |  zlib: ${s.zlibInflateLikely ? `yes (ratio ${s.zlibSizeRatio?.toFixed(3)})` : "no"}`);
