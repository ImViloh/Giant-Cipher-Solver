...
function onlyKeyLetters26(key) {
    return key
        .split("")
        .filter((c) => idx26(c) !== null)
        .join("");
}
function onlyKeyLetters52(key) {
    return key
        .split("")
...
export function vigenereDecryptA26(cipher, key) {
...
export function beaufortDecryptA26(cipher, key) {
    const k = onlyKeyLetters26(key);
    if (!k.length)
        return cipher;
    let ki = 0;
...
export function vigenereDecryptA52(cipher, key) {
...
export function beaufortDecryptA52(cipher, key) {
    const k = onlyKeyLetters52(key);
    if (!k.length)
        return cipher;
    let ki = 0;
...
export function atbashLettersA26(text) {
    let out = "";
...
export function caesarDecryptA26(text, shift) {
...
/** ROT13 convenience. */
export function rot13A26(text) {
    return caesarDecryptA26(text, 13);
}
/**
 * Rail fence decode: ciphertext was produced by reading the zigzag row-by-row.
 */
export function railFenceDecode(ciphertext, numRails) {
    const clean = ciphertext.replace(/\s/g, "");
    const len = clean.length;
    if (numRails < 2 || len < numRails)
        return ciphertext;
    const fence = Array.from({ length: numRails }, () => new Array(len).fill(null));
    let rail = 0;
    let direction = 1;
    for (let i = 0; i < len; i++) {
        fence[rail][i] = "*";
