...
import { isFullyEnglishReadable, englishReadabilityRank, } from "./englishReadability.js";
import { keyFromIndex, pow62 } from "./keyspace.js";
import { safeBase64Decode, vigenereBase64Decrypt, beaufortBase64Decrypt, xorBuffer, } from "./transforms.js";
...
export function bruteDeadline(t0, maxMs) {
    if (maxMs <= 0)
        return Number.MAX_SAFE_INTEGER;
    return t0 + maxMs;
}
export function getWorkerThreadCount() {
...
const xorWorkerFile = fileURLToPath(new URL("./workers/xorRangeWorker.js", import.meta.url));
const vigWorkerFile = fileURLToPath(new URL("./workers/vigRangeWorker.js", import.meta.url));
function tryDecodedBuffer(buf) {
...
function syncXorRange(decoded, len, rangeStart, rangeEnd, deadline, cumulativeBefore) {
    let tried = 0;
    let sinceTick = 0;
...
async function parallelXorRange(decoded, len, total, deadline, cumulativeBefore) {
 ...
 */
export async function bruteXorAlphanumeric(decoded, maxKeyLen, maxMs, onProgress) {
    const t0 = Date.now();
    ...
    for (let len = 1; len <= maxKeyLen; len++) {
        ...
        const useParallel = getWorkerThreadCount() > 1 && total >= PARALLEL_MIN_KEYS;
        let hit = null;
        let timedOut = false;
        let batchTried = 0;
        if (useParallel) {
            const r = await parallelXorRange(decoded, len, total, deadline, tried);
            hit = r.hit;
            batchTried = r.tried;
            timedOut = r.timedOut;
        }
        else {
            const r = syncXorRange(decoded, len, 0, total, deadline, tried);
            hit = r.hit;
            batchTried = r.tried;
            timedOut = r.timedOut;
        }
...
function syncVigRange(cipherB64, mode, len, rangeStart, rangeEnd, deadline, cumulativeBefore) {
    const fn = mode === "vigenere-b64" ? vigenereBase64Decrypt : beaufortBase64Decrypt;
    let tried = 0;
    let sinceTick = 0;
    for (let i = rangeStart; i < rangeEnd; i++) {
        if (Date.now() > deadline) {
...
async function parallelVigRange(cipherB64, mode, len, total, deadline, cumulativeBefore) {
    const threads = getWorkerThreadCount();
    const chunk = Math.ceil(total / threads);
    const workers = [];
    let tickAcc = 0;
 ...
 */
export async function bruteVigenereB64Alphanumeric(cipherB64, maxKeyLen, maxMs, onProgress) {
    const t0 = Date.now();
    const deadline = bruteDeadline(t0, maxMs);
    let tried = 0;
    const modes = [
    ...
    for (const mode of modes) {
        for (let len = 1; len <= maxKeyLen; len++) {
            ...
            let batchTried = 0;
            if (useParallel) {
                const r = await parallelVigRange(cipherB64, mode, len, total, deadline, tried);
                hit = r.hit;
                batchTried = r.tried;
                timedOut = r.timedOut;
            }
            else {
                const r = syncVigRange(cipherB64, mode, len, 0, total, deadline, tried);
                hit = r.hit;
                batchTried = r.tried;
                timedOut = r.timedOut;
            }
...
export function rankLatin1Text(text) {
