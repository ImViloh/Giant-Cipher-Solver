...
import { bufHashShort, collectNestedBase64, looksLikeBase64Text, rc4Transform, swapNibbles, tryHexAsciiDecode, tryZlibVariants, looksLikeHexAscii, } from "./layerTransforms.js";
...
function pushCand(out, method, detail, buf) {
 ...
 */
export function collectLayeredCandidates(decoded) {
    const out = [];
    const nestDepth = Math.min(12, Math.max(1, Number.parseInt(process.env.GIANT_NESTED_B64 ?? "8", 10) || 8));
    const maxBufs = Math.min(400, Math.max(8, Number.parseInt(process.env.GIANT_LAYER_MAX_BUFS ?? "140", 10) || 140));
    const maxXorRep = Math.min(220, Math.max(24, Number.parseInt(process.env.GIANT_LAYER_XORREP ?? "140", 10) || 140));
    const maxRc4 = Math.min(220, Math.max(24, Number.parseInt(process.env.GIANT_LAYER_RC4 ?? "140", 10) || 140));
    const doXor256 = (process.env.GIANT_LAYER_XOR256 ?? "1") !== "0";
    const xor256MaxBufs = Math.min(maxBufs, Math.max(0, Number.parseInt(process.env.GIANT_LAYER_XOR256_MAXBUFS ?? "56", 10) || 56));
    ...
    const seen = new Set();
    const layers = [];
    function addBuf(buf, path) {
        const h = bufHashShort(buf);
        if (seen.has(h))
            return;
        if (layers.length >= maxBufs)
            return;
        seen.add(h);
    ...
    for (let li = 0; li < layers.length; li++) {
        ...
        if (doXor256 && li < xor256MaxBufs) {
            for (let b = 0; b < 256; b++) {
                pushCand(out, "layer-xor1", `${path} key=0x${b.toString(16).padStart(2, "0")}`, xorSingleByte(buf, b));
            }
        }
        ...
        for (const k of rc4Keys) {
            pushCand(out, "layer-rc4", `${path} rc4 key="${k}"`, rc4Transform(buf, keyMaterial(k)));
        }
    }
    return out;
}
