...
import { renderCandidateCard, renderDumpIntro, renderHero, renderInputStrip, renderLogFooter, renderPhasesPipeline, renderReadabilityCard, renderStatsDashboard, renderWordHits, section, } from "./consoleUi.js";
...
function formatLine(rank, c) {
    ...
    const metaStr = m
        ...
        : "";
    return (`[${rank}] score=${c.score.toFixed(2)} | ${c.method} | ${c.detail}${metaStr}\n` +
        `    TEXT: ${truncateForDisplay(c.text)}`);
}
function truncateForDisplay(s, max = 2000) {
    if (s.length <= max)
        return s;
...
async function main() {
    ...
    const nearReadability = result.nearMiss
        ...
        : undefined;
    const plausibility = estimatePlausibilityPercent(result.solved, result.readability ?? nearReadability, result.nearMiss);
