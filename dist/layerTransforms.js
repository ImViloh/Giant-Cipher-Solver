...
export function rc4Transform(data, key) {
...
export function tryZlibVariants(buf) {
    if (buf.length < 4)
        return null;
    for (const fn of [gunzipSync, inflateSync, inflateRawSync]) {
        try {
            const r = fn(buf);
            if (r.length > 0 && r.length < buf.length * 50)
                return r;
        }
        catch {
            /* next */
...
export function looksLikeBase64Text(buf) {
    if (buf.length < 8)
        return false;
 ...
 */
export function collectNestedBase64(first, maxDepth) {
    const out = [{ buf: first, path: "b64[1]" }];
    let cur = first;
    let path = "b64[1]";
    for (let d = 1; d < maxDepth; d++) {
        if (!looksLikeBase64Text(cur))
            break;
...
export function tryHexAsciiDecode(buf) {
...
export function bufHashShort(buf) {
    return createHash("sha256").update(buf).digest("hex").slice(0, 16);
}
export function swapNibbles(buf) {
