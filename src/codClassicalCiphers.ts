...
export function modInv26(a: number): number | null {
  for (let x = 1; x < 26; x++) {
...
export function affineDecryptA26(ciphertext: string, a: number, b: number): string {
  const inv = modInv26(((a % 26) + 26) % 26);
  if (inv === null) return ciphertext;
  const bb = ((b % 26) + 26) % 26;
  let out = "";
...
function keyedAlphabetNoJ(keyPhrase: string): string {
...
function keyedAlphabet26(keyPhrase: string): string {
...
const polybiusSquareCache = new Map<
  string,
  { grid: string[][]; pos: Map<string, [number, number]> }
>();

function buildPolybiusSquare(keyword: string): { grid: string[][]; pos: Map<string, [number, number]> } {
...
export function playfairDecrypt(ciphertext: string, keyword: string): string {
  ...
  if (raw.length < 2) return ciphertext;
  let out = "";
 ...
 */
export function bifidDecrypt(ciphertext: string, keyword: string): string {
  const { grid, pos } = buildPolybiusSquare(keyword);
  ...
  for (const ch of letters) {
    const p = pos.get(ch);
    if (!p) return ciphertext;
    flat.push(p[0], p[1]);
  }
  if (flat.length !== 2 * n) return ciphertext;
...
export function columnarTranspositionDecrypt(ciphertext: string, keyword: string): string {
  ...
  if (k.length < 2) return ciphertext;
  const clean = ciphertext.toUpperCase().replace(/[^A-Z]/g, "");
  if (clean.length < k.length) return ciphertext;
  const cols = k.length;
  const rows = Math.ceil(clean.length / cols);
  const total = rows * cols;
  const padded = clean.padEnd(total, "X");
  const order = [...k.split("").map((c, i) => ({ c, i }))].sort((a, b) =>
    a.c === b.c ? a.i - b.i : a.c.localeCompare(b.c),
  );
...
export function keyedCaesarDecrypt(ciphertext: string, keyPhrase: string, shift: number): string {
  const keyed = keyedAlphabet26(keyPhrase);
  if (keyed.length < 26) return ciphertext;
...
export function vigenereDecryptCustomAlphabet(
  ciphertext: string,
  key: string,
  alphabet: string,
): string {
  const alpha = alphabet.toUpperCase().replace(/[^A-Z]/g, "");
  if (new Set(alpha).size !== 26) return ciphertext;
  const idxOf = new Map<string, number>();
  for (let i = 0; i < 26; i++) {
    idxOf.set(alpha[i]!, i);
  }
  const k = key.toUpperCase().replace(/[^A-Z]/g, "");
  if (!k.length) return ciphertext;
  let ki = 0;
  let out = "";
  for (const ch of ciphertext) {
    const u = ch.toUpperCase();
