...
export function getDictionaryWordSet(): Set<string> {
  return WORD_SET;
}

const COMMON_QUADGRAMS: Record<string, number> = {};
...
function countDictWords(text: string): { hits: number; tokens: number } {
  const lower = text.toLowerCase();
  const tokens = lower.split(/[^a-z]+/).filter((t) => t.length >= 2);
  let hits = 0;
  for (const t of tokens) {
    if (WORD_SET.has(t)) hits++;
  }
  return { hits, tokens: Math.max(1, tokens.length) };
}

function quadgramHitRatio(upper: string): number {
  if (upper.length < 4) return 0;
  let hits = 0;
  const max = upper.length - 3;
  for (let i = 0; i < max; i++) {
    const q = upper.slice(i, i + 4);
    if (COMMON_QUADGRAMS[q]) hits++;
  }
  return hits / max;
}
 ...
 */
export function isFullyEnglishReadable(text: string, minLen = 48): boolean {
  const r = analyzeEnglishReadability(text, minLen);
  return r.passes;
}

export function analyzeEnglishReadability(text: string, minLen = 48): ReadabilityReport {
  ...
  if (asciiRatio < 0.97) reasons.push("non_ascii");
  if (letterRatio < 0.52) reasons.push("low_letters");
  if (chiSq > 90) reasons.push(`chi_sq_${chiSq.toFixed(0)}`);
  if (hits < 12) reasons.push("few_dict_words");
  if (dictWordRatio < 0.12) reasons.push("low_dict_ratio");
  if (qHit < 0.012) reasons.push("low_quadgrams");
...
function fail(text: string, reasons: string[]): ReadabilityReport {
  let ascii = 0;
  let letters = 0;
  for (let i = 0; i < text.length; i++) {
    const c = text.charCodeAt(i);
...
export function englishReadabilityRank(text: string): number {
