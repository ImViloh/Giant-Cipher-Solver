...
function xorSingle(buf: Buffer, k: number): Buffer {
  const out = Buffer.alloc(buf.length);
  for (let i = 0; i < buf.length; i++) out[i] = buf[i]! ^ k;
  return out;
}

function scorePrintableRatio(buf: Buffer): number {
  if (buf.length === 0) return 0;
  let n = 0;
  for (let i = 0; i < buf.length; i++) {
    if (printableAsciiStrict(buf[i]!)) n++;
  }
  return n / buf.length;
}

/** Best single-byte XOR keys by strict printable ratio (for “try this next” hints). */
function topXorPrintableKeys(buf: Buffer, topN = 4): XorPrintableHint[] {
  if (buf.length === 0) return [];
  const scored: XorPrintableHint[] = [];
  for (let k = 0; k < 256; k++) {
...
function buildAnnotatedDump(buf: Buffer, maxBytes: number): HexDumpLine[] {
...
function topByteCounts(buf: Buffer, k: number): { hex: string; count: number; pct: number }[] {
  const c = new Uint32Array(256);
  for (let i = 0; i < buf.length; i++) c[buf[i]!]!++;
  const pairs: { b: number; count: number }[] = [];
  for (let b = 0; b < 256; b++) {
    if (c[b]! > 0) pairs.push({ b, count: c[b]! });
  }
...
function findAlignedDuplicateBlocks(
  buf: Buffer,
  blockSize: 8 | 16,
): HexPatternFind[] {
  const finds: HexPatternFind[] = [];
  if (buf.length < blockSize * 2) return finds;
...
function detectPeriod(buf: Buffer): HexPatternFind | null {
  ...
  if (bestP > 0 && bestScore > 0.35) {
    return {
      ...
      suggest:
        "Could be XOR with repeating key shorter than buffer, a patterned keystream, or coincidental — try Vigenère / repeating XOR in the solver.",
    };
  }
  return null;
}

function longestConstantRun(buf: Buffer): HexPatternFind | null {
...
function dominantDelta(buf: Buffer): HexPatternFind | null {
...
function constantXorNeighbour(buf: Buffer): HexPatternFind | null {
  if (buf.length < 6) return null;
  const x = buf[0]! ^ buf[1]!;
  for (let i = 1; i < buf.length - 1; i++) {
    if ((buf[i]! ^ buf[i + 1]!) !== x) return null;
  }
  return {
 ...
 */
export function analyzeHexPatterns(buf: Buffer): HexPatternReport {
  if (buf.length === 0) {
    return {
      annotatedLines: [],
      bytesAnnotated: 0,
  ...
  if (period) finds.push(period);

  const run = longestConstantRun(buf);
  if (run) finds.push(run);

  const delta = dominantDelta(buf);
  if (delta) finds.push(delta);

  const neigh = constantXorNeighbour(buf);
  if (neigh) finds.push(neigh);
